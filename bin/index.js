...
const run = (args) => {
    if (args.length >= 3) {
        ...
        if (ensureCommand(commandName)) {
            try {
                __webpack_require__(644)("./" + commandName + "/index.js").run(argv.splice(3));
            }
            catch (err) {
...
function showError(msg, help = false) {
    console.error((0, cmd_styles_1.error)("ERROR! " + (0, cmd_styles_1.warning)(msg)));
    if (help)
        showHelp();
}
function showWarning(msg, help = false) {
    console.error((0, cmd_styles_1.warning)("WARNING! " + (msg)));
    if (help)
        showHelp();
}
function showHelp() {
...
const ensureCommand = (commandName) => {
